<?php
    ...
    class Section extends CI_Model {
        ...
        public function getSections($documentation_id){
            ...
            try {
                ...
                if($get_documentation["status"] && $get_documentation["result"]){
                    ...
                    # Fetch sections
                    $get_sections = $this->db->query("SELECT id, documentation_id, title, description, updated_at FROM sections WHERE documentation_id = ? {$order_by_clause};", $documentation_id);
        ...
        # Last updated at: April 11, 2023
        # Owner: Erick, Updated by: Jovic
        public function getSection($params){
            $response_data = array("status" => false, "result" => array(), "error" => null);

            try {
                $query        = "SELECT id, documentation_id, title, description FROM sections WHERE id = ?";
                $bind_params  = $params["section_id"];

                if(isset($params["documentation_id"])){
                    $query = "SELECT sections.id, sections.documentation_id, sections.title, sections.description
                                FROM documentations
                                INNER JOIN sections ON sections.id = ? AND sections.documentation_id = documentations.id
                                WHERE documentations.id = ?";
                    
                    $bind_params = array($params["section_id"], $params["documentation_id"]);

                    # Add condition for User
                    if($_SESSION["user_level_id"] == USER_LEVEL["USER"]){
                        $query .= " AND documentations.is_archived = ?";
                        array_push($bind_params, FALSE_VALUE);
                    }
                }

                $get_section = $this->db->query($query, $bind_params);
        ...
        public function getSectionTabs($section_id, $module_id = ZERO_VALUE){
            ...
            try {
                ...
                if($_SESSION["user_level_id"] == USER_LEVEL["USER"]){
                    $where_statement .= "AND (documentations.is_private = ? OR documentations.id IN (SELECT collaborators.documentation_id FROM collaborators WHERE collaborators.user_id = ?))";
                    array_push($where_values, FALSE_VALUE, $_SESSION["user_id"]);
                }
                ...
                $get_section = $this->db->query("
                    ...
                    FROM sections
                    INNER JOIN documentations ON documentations.id = sections.documentation_id
